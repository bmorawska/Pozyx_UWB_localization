...
from pypozyx import (PozyxConstants, Coordinates, POZYX_SUCCESS, PozyxRegisters, version,
                     DeviceCoordinates, PozyxSerial, get_first_pozyx_serial_port, SingleRegister, DeviceRange)
from pythonosc.udp_client import SimpleUDPClient
from pypozyx.tools.version_check import perform_latest_version_check

from devices import anchors
...
class MultitagPositioning(object):
    ...
    def __init__(self, pozyx, osc_udp_client, tag_ids, anchors, algorithm=PozyxConstants.POSITIONING_ALGORITHM_UWB_ONLY,
                 dimension=PozyxConstants.DIMENSION_3D, height=1000, ranging=False):
        self.pozyx = pozyx
    ...
    def loop(self):
        ...
        for tag_id in self.tag_ids:
            ...
            if self.ranging and tag_id is not None:
                device_range = DeviceRange()
                status = self.pozyx.doRanging(tag_id, device_range, None)
                if status == POZYX_SUCCESS:
                    print(device_range)
                else:
...
if __name__ == "__main__":
    ...
    anchors = [DeviceCoordinates(anchors[0], 1, Coordinates(0, 0, 2210)),
               DeviceCoordinates(anchors[1], 1, Coordinates(9200, -700, 2400)),
               DeviceCoordinates(anchors[2], 1, Coordinates(0, 17000, 2470)),
               DeviceCoordinates(anchors[3], 1, Coordinates(2700, 17000, 2470))]
    ...
    r = MultitagPositioning(pozyx, osc_udp_client, tag_ids, anchors,
                            algorithm, dimension, height, ranging)
